...
2. **Acrescente** o bloco do cérebro, delimitado por marcadores, sem apagar o que já existe:
   ```
   <!-- context-keeper:inicio -->
   ...
   <!-- context-keeper:fim -->
   ```
   Os marcadores permitem que o Modo 5 atualize ou remova o bloco depois.
3. Registre o arquivo alterado em `.context-keeper/config.json` → `integrations`.
...
Primeiro, garanta que o ponteiro `~/.context-keeper/config` existe (`brain_path=<CEREBRO>`). Depois, depende de como a skill foi instalada:

**Instalada como plugin (recomendado).** O plugin `context-keeper` já traz os hooks em `hooks/hooks.json`. Eles ficam inativos até o ponteiro existir e passam a funcionar na próxima sessão. Não edite `~/.claude/settings.json`.

**Instalada como skill avulsa.**
1. Copie `scripts/*.sh` da skill (incluindo `lib.sh`) para `<CEREBRO>/.context-keeper/scripts/`.
...
```bash
echo '{"source":"startup"}' | bash "<pasta-dos-scripts>/session-start.sh"
```
A saída deve ser curta e conter o arquivo de estado atual (`AGORA.md`).

O que cada hook faz:
- **SessionStart** (`session-start.sh`): roda ao abrir, retomar, limpar ou **compactar** a sessão. O que ele imprime (até 10.000 caracteres; o script corta em 8.000) entra no contexto da IA. Após uma compactação, acrescenta um aviso para a IA retomar pelo `AGORA.md`.
- **Stop** (`checkpoint-stop.sh`): roda quando a IA termina uma resposta. Se a transcrição cresceu mais que o limite (padrão ~300 KB) desde o último checkpoint, devolve um `additionalContext` pedindo que a IA faça um checkpoint antes de parar — aparece para o usuário como "Stop hook feedback", não como erro. Tem proteção contra laço (`stop_hook_active`) e só dispara de novo após novo crescimento. O limite é ajustável pela variável `CONTEXT_KEEPER_CHECKPOINT_BYTES`.

Os nomes dos arquivos (`CEREBRO.md`/`BRAIN.md`, `AGORA.md`/`NOW.md`, `Diario/`/`Journal/`) vêm de `files` em `.context-keeper/config.json`, então os scripts funcionam em qualquer idioma e em cérebros adotados (ex.: raiz `Claude.md`).
