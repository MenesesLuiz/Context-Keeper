...
- [x] **Phase 4 — Real tests (2026-09-25):** with-skill 97% vs baseline 71% on the persona scenarios; adopt end to end passed; real Claude Code session with the plugin loaded the brain and recorded the checkpoint baseline from real token usage.
  - [x] load the plugin in a real session (`claude --plugin-dir .`);
  - [x] adopt a hand-made brain end to end (persona `leo-adota`);
  - [x] run the scenarios in `tests/skill-evals.json` with and without the skill, against the fictional personas from `tests/make-personas.sh` (each test only sees its persona's simulated home);
  - [x] fixes from the tests: sandboxed detection, adopt rules, 4 questions per round, About-me and projects-map templates, lint ignores project code and needs no temp files;
  - [x] projects organized inside the brain (advice only: the AI recommends, the user moves).
